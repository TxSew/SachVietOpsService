...
export class OtpService {
    ...
    async verifyOtp(otp: number, token: string): Promise<{ reset_password_token: string }> {
        try {
            if (!token) {
            ...
            if (error.name === 'TokenExpiredError') {
                throw ResponseError.unauthorized('Token has expired, please get a new token');
            }
            console.log(error);
            throw error;
        }
    }
    async resetPassword(token: string, newPasswordDTO: NewPasswordDTO): Promise<{ message: string }> {
        try {
            if (!token) {
            ...
            if (error.name === 'TokenExpiredError') {
                throw new UnauthorizedException('Token has expired, please get a new token');
            }
            throw error;
        }
    }
